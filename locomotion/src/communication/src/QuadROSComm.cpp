...
void QuadROSComm::initClass() {
    ...
    if (m_mode == DATA_ACCESS_MODE::EXECUTOR) {
        ...
        m_joy_sub = this->create_subscription<sensor_msgs::msg::Joy>(
            "/" + m_name + "/joystick_cmd", 
            1, 
            std::bind(&QuadROSComm::get_joystick_data_cb, this, std::placeholders::_1)
        );
        for (int i = 0; i < 4; ++i) {
    ...
    joystick_data = sensor_msgs::msg::Joy();
...
void QuadROSComm::get_joystick_data_cb(const sensor_msgs::msg::Joy::SharedPtr msg) const {
    m_joystick_data_ptr -> vel_x = msg -> axes[0];
    m_joystick_data_ptr -> vel_y = msg -> axes[1];
    m_joystick_data_ptr -> vel_yaw = msg -> axes[2];
    m_joystick_data_ptr -> mode = msg -> axes[3];
...
void QuadROSComm::start_thread() {
    m_thread = std::thread(&QuadROSComm::run, this);
    m_thread.detach();
}